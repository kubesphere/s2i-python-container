...
This container image includes Python 3.8 as a [S2I](https://github.com/kubesphere/source-to-image) base image for your Python 3.8 applications.
...
For this, we will assume that you are using the supported image, available via `python:3.8` imagestream tag in kubesphere.
Building a simple [python-sample-app](https://github.com/sclorg/django-ex.git) application
in kubesphere can be achieved with the following step:
...
-------------------------------------
This image supports the [Source-to-Image](https://docs.kubesphere.com/container-platform/3.11/creating_images/s2i.html)
(S2I) strategy in kubesphere. The Source-to-Image is an kubesphere framework
...
when you build the image outside of the kubesphere environment.
...
# Add application sources with correct permissions for kubesphere
USER 0
ADD app-src .
RUN chown -R 1001:0 ./
USER 1001
...
* **DISABLE_MIGRATE**
    ...
    "Handling Database Migrations" section of [Django blogpost on kubesphere blog](
    https://blog.kubesphere.com/migrating-django-applications-kubesphere-3/) on suggestions
    how/when to run DB migrations in kubesphere environment. Most importantly,
